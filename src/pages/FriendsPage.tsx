  ...
  return (
    <div className="space-y-5">
        ...
        {tabs.map((tab) => (
          ...
          >
            ...
            {tab.count !== undefined && tab.count > 0 && (
              <span className="bg-accent/15 text-accent text-[10px] font-semibold px-1.5 py-0.5 rounded-lg min-w-[20px] text-center">
                {tab.count}
              </span>
            )}
          </button>
      ...
      {activeTab === "friends" && (
        <div className="space-y-2">
          {friendsLoading && (
            <div className="space-y-2">
              {[1, 2, 3].map((n) => (
                <div key={n} className="surface rounded-xl border border-border p-4 animate-pulse flex items-center gap-3">
          ...
          {!friendsLoading && friends.length === 0 && (
            <div className="surface rounded-xl border border-border p-12 text-center">
          ...
          {friends.map((friend, i) => (
            <motion.div
              ...
              className="surface rounded-xl border border-border p-4 flex items-center gap-3 hover:shadow-elevated hover:border-accent/15 transition-all"
            >
              <div className="w-11 h-11 rounded-xl bg-accent/10 flex items-center justify-center text-accent font-semibold text-sm shrink-0">
              ...
              <button
                ...
                className="flex items-center gap-1.5 text-[12px] font-medium text-accent bg-accent/10 hover:bg-accent/20 px-3 py-2 rounded-lg transition-colors mr-1"
              >
              ...
              <button
                ...
                className="text-[12px] text-muted-foreground hover:text-destructive transition-colors px-2.5 py-2 rounded-lg hover:bg-destructive/5"
              >
                Unfriend
              </button>
            </motion.div>
      ...
      {activeTab === "incoming" && (
        <div className="space-y-2">
          {incomingLoading && (
            <div className="surface rounded-xl border border-border p-4 animate-pulse flex items-center gap-3">
              <div className="w-11 h-11 rounded-xl bg-muted" />
          ...
          {incoming.map((req, i) => (
            <motion.div
              ...
              animate={{ opacity: 1, y: 0 }}
              transition={{ delay: i * 0.04 }}
              className="surface rounded-xl border border-border p-4 flex items-center gap-3"
            >
              <div className="w-11 h-11 rounded-xl bg-accent/10 flex items-center justify-center text-accent font-semibold text-sm shrink-0">
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="flex items-center gap-1.5 text-[12px] font-medium text-success bg-success/10 hover:bg-success/20 px-3 py-2 rounded-lg transition-colors disabled:opacity-50"
                >
                ...
                <button
                  ...
                  className="flex items-center gap-1.5 text-[12px] font-medium text-muted-foreground hover:text-destructive px-3 py-2 rounded-lg transition-colors hover:bg-destructive/5"
                >
        ...
        <div className="space-y-3">
          <div className="relative max-w-sm">
            ...
            <input
              ...
              className="w-full h-10 pl-9 pr-3 rounded-lg border border-input bg-surface text-[13px] text-foreground placeholder:text-muted-foreground/60 focus:outline-none focus:ring-2 focus:ring-accent/20 focus:border-accent transition-all"
            />
          </div>
            ...
            <div className="space-y-2">
              {searchResults.map((user, i) => (
                <motion.div
                  ...
                  animate={{ opacity: 1, y: 0 }}
                  transition={{ delay: i * 0.04 }}
                  className="surface rounded-xl border border-border p-4 flex items-center gap-3 hover:shadow-elevated transition-all"
                >
                  <div className="w-11 h-11 rounded-xl bg-accent/10 flex items-center justify-center text-accent font-semibold text-sm shrink-0">
                  ...
                  {user.is_friend ? (
                    <span className="text-[12px] text-success flex items-center gap-1.5 bg-success/10 px-2.5 py-1.5 rounded-lg">
                      <UserCheck className="w-3.5 h-3.5" /> Friends
                    </span>
                  ) : user.pending_request_id ? (
                    <span className="text-[12px] text-muted-foreground flex items-center gap-1.5 bg-muted px-2.5 py-1.5 rounded-lg">
                    ...
                    <button
                      ...
                      className="flex items-center gap-1.5 text-[12px] font-medium text-accent bg-accent/10 hover:bg-accent/20 px-3 py-2 rounded-lg transition-colors disabled:opacity-50"
                    >
